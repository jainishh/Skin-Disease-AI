...
async def download_report(
    ...
    try:
        ...
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "Invalid prediction ID.")
    ...
    recommendation = get_recommendations(clean_disease_id, prediction.get("severity", "Mild"))
    ...
    if resolved_email == "Optional" and current_user and current_user.get("email"):
        resolved_email = current_user["email"]

    patient_name = current_user.get("full_name", "Patient") if current_user else "Patient"
...
async def share_report_email(
    ...
    recommendation = get_recommendations(prediction["primary_disease"], prediction.get("severity", "Mild"))
...
async def download_fhir_report(
    ...
    user_id_str = str(current_user["_id"]) if current_user else "guest"
    user_name_str = current_user.get("full_name", "Unknown Patient") if current_user else "Guest Patient"
    ...
    fhir_observation = {
        ...
        "subject": {
            "reference": f"Patient/{user_id_str}",
            "display": user_name_str
        },
...
@router.get("/{prediction_id}/econsult-handout")
async def download_econsult_handout(
    prediction_id: str,
    token: Optional[str] = Query(None),
    current_user: Optional[dict] = Depends(get_current_user_optional)
):
    ...
    recommendation = get_recommendations(clean_disease_id, prediction.get("severity", "Mild"))
    nearby_doctors = await find_doctors(limit=3)
    nearby_hospitals = await find_hospitals(limit=3)
    verification_url = f"https://yourapp.example.com/verify-report/{prediction_id}"

    patient_name = current_user.get("full_name", "Patient") if current_user else "Patient"
    patient_email = current_user.get("email", "Optional") if current_user else "Optional"
    ...
    return Response(
        content=pdf_bytes,
        media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=econsult_handout_{prediction_id}.pdf"},
    )
