...
@router.post("/triage")
async def evaluate_clinical_triage(data: dict):
    disease = data.get("disease", "General")
    severity = data.get("severity", "Mild")
    itching = bool(data.get("itching", False))
    bleeding = bool(data.get("bleeding", False))
    rapid_growth = bool(data.get("rapid_growth", False))
    pain_level = int(data.get("pain_level", 0))
    duration_days = int(data.get("duration_days", 1))

    score = 0
    if severity == "Severe" or disease in ["Melanoma", "Basal_Cell_Carcinoma"]:
        score += 3
    ...
    if score >= 5:
        level = 3
        badge = "Urgent Specialist Triage Required"
        color = "rose"
        timeframe = "Within 24 to 48 Hours"
        summary = "High clinical priority detected due to severe presentation or warning signs (bleeding, rapid growth, intense pain)."
        actions = [
            "Schedule an immediate consultation with a certified dermatologist.",
            "Avoid scratching, rubbing, or applying unprescribed harsh chemical ointments.",
            "Bring this AI Diagnostic Report and Grad-CAM visual to your appointment."
        ]
    elif score >= 3:
        level = 2
        badge = "Prompt Consultation Recommended"
        color = "amber"
        timeframe = "Within 1 to 2 Weeks"
        summary = "Moderate clinical concern. Symptoms require professional evaluation to prevent secondary infection or chronic flare."
        actions = [
            "Book a non-urgent clinic visit with a local dermatologist.",
            "Maintain daily barrier moisturizing and gentle skin cleansing.",
            "Monitor lesion margins weekly using the AI Progression Tracker."
        ]
    else:
        level = 1
        badge = "Routine Home Monitoring"
        color = "emerald"
        timeframe = "Routine / Self-Care"
        summary = "Mild/low-risk skin presentation. Continue baseline skincare and observe for any changes."
        actions = [
            "Keep the affected skin area clean, hydrated, and protected from solar exposure.",
            "Re-scan using AI system if lesion changes size, color, or shape."
        ]
