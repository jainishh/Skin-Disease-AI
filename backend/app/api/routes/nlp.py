...
async def analyze_ingredients(
    ...
    text = payload.ingredients_text.lower()
    condition = payload.condition or "General"
    ...
    beneficial_dict = {
        "ceramide": "Barrier restoration & deep hydration",
        "niacinamide": "Anti-inflammatory & redness reduction",
        "hyaluronic acid": "Humectant moisture retention",
        "glycerin": "Gentle skin hydrating agent",
        "zinc oxide": "Calming physical UV filter & skin barrier protectant",
        "panthenol": "Pro-Vitamin B5 soothing agent",
        "centella": "Centella Asiatica skin repair extract",
        "colloidal oatmeal": "FDA-approved anti-itch soothing agent for eczema"
    }

    harmful_dict = {
        "alcohol denat": "Drying alcohol that breaks down the skin barrier",
        "denatured alcohol": "Severe skin barrier drying agent",
        "fragrance": "Potential contact allergen and irritant",
        "parfum": "Unspecified synthetic fragrance allergen",
        "sodium lauryl sulfate": "Harsh detergent surfactant causing striping & irritation",
        "sls": "Harsh skin stripping surfactant",
        "essential oil": "Volatile botanical aromatic allergen",
        "formaldehyde": "Sensitizing chemical preservative"
    }

    comedogenic_dict = {
        "isopropyl myristate": "Highly comedogenic pore-clogging ingredient",
        "coconut oil": "Heavy comedogenic oil prone to causing acne breakouts",
        "lanolin": "Heavy occlusive wax that may trigger allergic reactions in sensitive skin"
    }

    for key, desc in beneficial_dict.items():
        if key in text:
            beneficial_list.append({"ingredient": key.title(), "benefit": desc})

    for key, desc in harmful_dict.items():
        if key in text:
            concerning_list.append({"ingredient": key.title(), "reason": desc, "severity": "High"})

    for key, desc in comedogenic_dict.items():
        if key in text:
            concerning_list.append({"ingredient": key.title(), "reason": desc, "severity": "Moderate"})

    score = 100 - (len(concerning_list) * 20) + (len(beneficial_list) * 5)
    score = max(10, min(100, score))

    if condition == "Eczema" and any(c["ingredient"].lower() in ["alcohol denat", "fragrance", "parfum", "sodium lauryl sulfate"] for c in concerning_list):
        warnings.append("⚠️ Contains harsh drying agents or fragrance that can trigger severe Eczema flare-ups.")
    if condition == "Acne" and any(c["ingredient"].lower() in ["isopropyl myristate", "coconut oil"] for c in concerning_list):
        warnings.append("⚠️ Contains heavy pore-clogging oils that exacerbate Acne lesions.")
    ...
    return {
        ...
        "concerning_count": len(concerning_list),
        "beneficial_ingredients": beneficial_list,
        "concerning_ingredients": concerning_list,
        "warnings": warnings
    }
