...
import { Camera, ImagePlus, RefreshCw, X, Zap, Sliders, Sparkles, CheckCircle2, Shield, Leaf, AlertTriangle } from "lucide-react";
      ...
      <AnimatePresence mode="wait">
        {!result ? (
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="mt-8 bg-white dark:bg-[#1E293B] border border-slate-100 dark:border-[#334155] rounded-[2.5rem] p-8 max-w-2xl mx-auto shadow-[0_16px_40px_rgba(240,242,245,0.4)] text-left"
          >
            <div className="flex flex-col md:flex-row gap-6 items-center md:items-start pb-6 border-b border-slate-100 dark:border-[#334155]/40">
              {file && (
                <img
                  src={URL.createObjectURL(file)}
                  alt="Scanned lesion"
                  className="h-32 w-32 rounded-[1.5rem] object-cover bg-slate-50 border border-slate-100 dark:border-[#334155]"
                />
              )}
              <div className="flex-1 space-y-3 text-center md:text-left w-full">
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <span className="bg-blue-550/10 text-blue-600 dark:text-blue-400 text-[10px] px-2.5 py-1 rounded-lg font-bold uppercase tracking-wider bg-blue-50 dark:bg-blue-950/20">
                    {t('results.primary_prediction')}
                  </span>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 bg-slate-100 dark:bg-[#0B1220] rounded-full h-2.5 overflow-hidden">
                    <div 
            ...
            {loadingRec ? (
              <p className="text-slate-400 text-xs py-8 text-center animate-pulse">{t('common.loading')}</p>
            ) : recommendation ? (
              <div className="py-6 space-y-5">
                <div>
                  <h4 className="text-xs font-bold text-slate-450 dark:text-[#94A3B8] uppercase tracking-wider mb-2">{t('results.skin_care')}</h4>
                  <ul className="space-y-1.5">
                    {recommendation.skin_care?.map((item: string, i: number) => (
                      <li key={i} className="text-xs text-slate-600 dark:text-[#CBD5E1] flex items-start gap-2">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-450 dark:text-[#94A3B8] uppercase tracking-wider mb-2">{t('results.lifestyle')}</h4>
                  <ul className="space-y-1.5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <div className="bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/10 dark:border-emerald-500/20 rounded-2xl p-4">
                    <h5 className="text-[10px] font-bold text-emerald-700 dark:text-emerald-450 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                      <Leaf size={12} /> {t('results.diet_eat')}
                    </h5>
                    <ul className="space-y-1">
                      {recommendation.diet_eat?.map((item: string, i: number) => (
                        <li key={i} className="text-2xs text-slate-600 dark:text-[#CBD5E1] flex items-start gap-1">
                  ...
                  <div className="bg-rose-500/5 dark:bg-rose-500/10 border border-rose-500/10 dark:border-rose-500/20 rounded-2xl p-4">
                    <h5 className="text-[10px] font-bold text-rose-700 dark:text-rose-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                      <AlertTriangle size={12} /> {t('results.diet_avoid')}
                    </h5>
                    <ul className="space-y-1">
            ...
            <div className="flex gap-3 pt-6 border-t border-slate-100 dark:border-[#334155]/40">
              <button 
                onClick={() => {
                  setResult(null);
                  setFile(null);
                ...
                className="flex-1 py-3.5 bg-blue-500 hover:bg-blue-600 text-white rounded-2xl text-xs font-semibold text-center shadow-md transition"
              >
