  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0F172A] flex font-body text-slate-800 dark:text-[#CBD5E1] transition-all duration-300">
      ...
      <div className="w-20 bg-white dark:bg-[#0B1220] border-r border-slate-100 dark:border-[#334155] flex flex-col items-center py-8 justify-between shrink-0 z-10 shadow-[4px_0_24px_rgba(240,242,245,0.4)] dark:shadow-none transition-colors duration-300">
        ...
        <div className="flex flex-col items-center gap-2 p-1.5 bg-slate-50 dark:bg-[#1E293B]/40 border border-slate-100 dark:border-[#334155]/60 rounded-2xl w-12 shadow-inner mb-2">
          {/* Language Selector */}
          <div className="flex flex-col gap-1 w-full">
            {[{code:'en',label:'EN'},{code:'hi',label:'हि'},{code:'gu',label:'ગુ'}].map(l => (
              <button
                ...
                className={`h-7 w-full rounded-xl text-[9px] font-bold transition-all ${
                  currentLang === l.code
                    ? 'bg-blue-500 text-white shadow-sm'
                    : 'text-slate-400 dark:text-slate-500 hover:bg-white dark:hover:bg-[#1E293B] hover:text-slate-705 dark:hover:text-slate-350'
                }`}
              >{l.label}</button>
            ))}
          </div>

          <div className="h-px bg-slate-200/80 dark:bg-[#334155]/40 w-full" />

          {/* Settings button */}
          <button 
            onClick={() => setActiveTab("profile")} 
            title={t('common.settings') || "Settings"}
            className="h-7 w-full flex items-center justify-center rounded-xl text-slate-400 dark:text-[#94A3B8] hover:bg-white dark:hover:bg-[#1E293B] hover:text-slate-700 dark:hover:text-[#F8FAFC] transition"
          >
            <Settings size={14} />
          </button>
        </div>
      ...
      <div className="flex-1 grid grid-cols-1 xl:grid-cols-12 overflow-hidden">
        ...
        <div className="xl:col-span-8 p-6 md:p-8 overflow-y-auto space-y-8 max-w-5xl">
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-slate-400 dark:text-[#94A3B8] text-xs font-semibold uppercase tracking-wider">{t('dashboard.section_label')} / {t(`dashboard.tab_${activeTab}`)}</p>
              <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-[#F8FAFC] mt-0.5">
            ...
            <div className="flex items-center gap-3">
              <div className="relative hidden md:block">
                ...
                <input 
                  type="text" 
                  placeholder={t('dashboard.search_symptoms_placeholder')}
            ...
            {activeTab === "home" && (
              ...
              >
                ...
                <div className="bg-white dark:bg-[#1E293B] border border-slate-100 dark:border-[#334155] rounded-[2.5rem] p-6 flex flex-col md:flex-row gap-6 items-center justify-between shadow-sm">
                  <div className="flex-1 space-y-4">
                    <h2 className="text-xl font-bold text-slate-900 dark:text-[#F8FAFC] flex items-center gap-2">
                      <User size={20} className="text-blue-500" /> {t('dashboard.lesion_map_title')}
                    </h2>
                    <p className="text-xs text-slate-500 dark:text-[#94A3B8]">
                      {t('dashboard.lesion_map_desc')}
                    </p>
                    <div className="flex gap-3 mt-2">
                      <span className="flex items-center gap-1.5 text-[10px] text-slate-500 font-bold"><div className="h-2 w-2 rounded-full bg-emerald-500" /> {t('dashboard.mild')}</span>
                      <span className="flex items-center gap-1.5 text-[10px] text-slate-500 font-bold"><div className="h-2 w-2 rounded-full bg-amber-500" /> {t('dashboard.moderate')}</span>
                      <span className="flex items-center gap-1.5 text-[10px] text-slate-500 font-bold"><div className="h-2 w-2 rounded-full bg-rose-500" /> {t('dashboard.severe')}</span>
                    </div>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-36 bg-slate-900 dark:bg-slate-700 text-white text-[10px] rounded-xl p-2.5 opacity-0 group-hover:opacity-100 transition pointer-events-none z-20 shadow-xl text-center">
                            ...
                            <p className="text-[9px] text-slate-300">{t('dashboard.confidence_pct', { pct: Math.round(item.confidence * 100) })} • {t(`dashboard.${item.severity.toLowerCase()}`)}</p>
                ...
                <div className="w-full">
                  ...
                  <div className="relative overflow-hidden rounded-[2rem] bg-violet-50 dark:bg-violet-950/10 border border-violet-100 dark:border-violet-900/30 p-7 min-h-[190px] flex flex-col justify-between shadow-[0_12px_24px_rgba(240,242,245,0.4)] dark:shadow-none">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="text-violet-900 dark:text-violet-200 font-semibold text-base leading-tight">{t('dashboard.found_doctors')}</h3>
                        <p className="text-violet-600/70 dark:text-violet-400/70 text-xs mt-1">{t('dashboard.doctors_nearby', { count: displayedDoctors.length })}</p>
                      </div>
                    ...
                    <div className="relative h-20 w-full rounded-2xl bg-white/60 dark:bg-[#1E293B]/40 border border-violet-200/20 dark:border-violet-800/20 mt-3 p-3 flex items-center justify-between overflow-hidden shadow-inner">
                      ...
                      <div className="z-10 flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-violet-900 dark:text-violet-200 font-semibold text-xs leading-none">{displayedDoctors[0]?.clinic_name || "Aesthetic Skin Clinic"}</p>
                          <p className="text-[10px] text-violet-600 dark:text-violet-400 mt-1">{t('dashboard.doctor_distance', { city: displayedDoctors[0]?.city || "Bandra" })}</p>
                        </div>
                      </div>
                ...
                <div className="space-y-4">
                  <h2 className="text-xl font-semibold tracking-tight text-slate-900 dark:text-[#F8FAFC]">{t('dashboard.quick_actions')}</h2>
                    ...
                    {[
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-slate-400 dark:text-[#94A3B8] text-[10px] mt-1">{t(`dashboard.${cat.subKey}`)}</p>
                        </div>
                      </Link>
                    ))}
                  </div>
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <div className="bg-white dark:bg-[#1E293B] border border-slate-100 dark:border-[#334155] rounded-[2.5rem] p-6 shadow-sm">
                    <h2 className="text-lg font-semibold tracking-tight text-slate-900 dark:text-[#F8FAFC] mb-4 flex items-center gap-2">
                      <Clock size={18} className="text-blue-500" /> {t('dashboard.recent_analyses')}
                    </h2>

                    {loading ? (
                      <p className="text-slate-400 text-xs py-8 text-center animate-pulse">{t('dashboard.loading_scans')}</p>
                    ) : history.length === 0 ? (
                  ...
                  <div className="bg-white dark:bg-[#1E293B] border border-slate-100 dark:border-[#334155] rounded-[2.5rem] p-6 shadow-sm">
                    <h2 className="text-lg font-semibold tracking-tight text-slate-900 dark:text-[#F8FAFC] mb-4 flex items-center gap-2">
                      <Calendar size={18} className="text-blue-500" /> {t('dashboard.appointments')}
                    </h2>
                ...
                <div className="space-y-4">
                  <h2 className="text-xl font-semibold tracking-tight text-slate-900 dark:text-[#F8FAFC]">{t('dashboard.top_specialists')}</h2>
              ...
              >
                ...
                <div className="relative pl-6 sm:pl-8 border-l-2 border-slate-100 dark:border-[#334155]/60 ml-4 py-2 space-y-8">
                  {filteredHistory.length > 0 ? (
                          ...
                          >
                            ...
                            <div className="rounded-3xl border border-slate-100 dark:border-[#334155] bg-white dark:bg-[#1E293B] p-5 shadow-xs hover:shadow-md transition-all duration-300 flex flex-col md:flex-row md:items-center justify-between gap-5">
                              <div className="flex items-start gap-4">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                    <span className="text-xs font-bold text-blue-500">{t('dashboard.confidence_pct', { pct: Math.round(item.confidence * 100) })}</span>
                                    <SeverityBadge severity={item.severity} />
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-3 pt-3 md:pt-0 border-t md:border-t-0 border-slate-50 dark:border-[#334155]/20 justify-end">
                                ...
                                >
                                  <FileText size={14} /> {t('results.download_report')}
                                </button>
                    ...
                    <p className="text-center py-12 text-slate-400 text-xs w-full ml-[-20px]">{t('dashboard.no_scans')}</p>
                  )}
                </div>
              </motion.div>
            )}
        ...
        <div className="xl:col-span-4 bg-white dark:bg-[#1E293B] border-l border-slate-100 dark:border-[#334155] p-6 md:p-8 space-y-8 flex flex-col justify-between overflow-y-auto transition-colors duration-300">
          <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/30 text-blue-600 dark:text-blue-400 text-[9px] px-2 py-1 rounded-md font-bold uppercase">{t('dashboard.today')}</span>
            </div>
            ...
            <div className="bg-slate-50/50 dark:bg-[#273449]/30 border border-slate-100 dark:border-[#334155]/60 rounded-3xl p-5 shadow-xs space-y-4">
              <p className="text-[10px] font-bold text-slate-400 dark:text-[#94A3B8] uppercase tracking-wider">{t('dashboard.medication_reminders')}</p>
            ...
            <div className="bg-slate-50/50 dark:bg-[#273449]/30 border border-slate-100 dark:border-[#334155]/60 rounded-3xl p-5 shadow-xs space-y-4">
              <p className="text-[10px] font-bold text-slate-400 dark:text-[#94A3B8] uppercase tracking-wider">{t('dashboard.clinical_alerts')}</p>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-3 p-3 bg-blue-500/10 border border-blue-500/20 rounded-2xl">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-blue-800 dark:text-blue-400">{t('dashboard.next_assessment')}</h4>
                    <p className="text-[10px] text-slate-600 dark:text-[#94A3B8] leading-relaxed mt-1">
